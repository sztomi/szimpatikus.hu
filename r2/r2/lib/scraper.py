...
class OEmbed(Scraper):
    ...
    def media_object(self):
        ...
        if self.oembed and self.oembed.get('type') in ['video', 'rich', 'photo']:
    ...
    def media_embed(cls, video_id = None, height = None, width = None, **kw):
        ...
        elif oembed and oembed.get('type') == 'photo' and oembed.get('url'):
    	    content = '<img src=\"' + oembed.get('url') + '\" border=\"0\" style=\"max-width: 800px; max-height: 600px;\"\/>'
        if content and oembed.get('height') and oembed.get('width'):
    	    if oembed['width'] > 800:
    		oembed['width'] = 800
    	    if oembed['height'] > 600:
    		oembed['height'] = 600
